...
class ImageHandler:
    ...
    def prefetch_images(self, depth=3, max_prefetch=10):
        ...
        # Interleave ahead and behind indices for optimal prefetching
        prefetch_indices = [item for pair in zip(ahead, behind) for item in pair]

        # Limit the number of prefetch items
        ...
        # Prefetch images based on calculated indices
                        ...
                        if image_files:
                            with self.lock:
                                ...
                                self.data_service.set_image_list(
                                    image_list)  # Update the image list in the data service
                    except Exception as e:
