...
class ImageManager(QObject):
    ...
    def refresh_image_list(self):
        ...
        """
        if self.is_shutting_down():
            logger.debug("[ImageManager] Shutdown initiated, not starting new refresh task.")
            return
        self.event_bus.emit('show_busy')
    ...
    def on_image_list_updated(self):
        ...
        if not all((self.is_loading, self.is_loaded)) and \
    ...
    def undo_last_action(self):
        ...
        if last_action:
            logger.debug(f"[ImageManager] Undo last action for image: {last_action[1]}")
            self.display_image()
        else:
